...
# create_tbl_news_sql = "create table news (  news_dt date, city varchar(200), text varchar(4000));"
# create_tbl_ad_sql = "create table ad ( ad_txt varchar(4000), exp_date date, exp_date_txt as varchar(4000) );"
# create_tbl_fib_sql = "create table fib (  nr int, fib_nr_txt as varchar(4000) );"
# creating tables
# cursor.execute(create_tbl_news_sql)
# cursor.execute(create_tbl_ad_sql)
# cursor.execute(create_tbl_fib_sql)
# db inserts are made only for file_input_type == "m" - manual record creation, because it is the same for file/csv/json
# duplicate check is done only for news because it is the same principle for all 3 record types
...
if file_input_type == "m":
    ...
    while loop_marker == "yes":
        ...
        if input_type == "news":
            ...
            dupe_check_sql = "select concat(news_dt,city,text), count(concat(news_dt,city,text)) from news group by concat(news_dt,city,text) having count(concat(news_dt,city,text))>1"  # checking duplicates, done only for news because for text and fib is the same, count group by having, executing insert only if count =0
            # results = cursor.execute(dupe_check_sql).fetchall()
            # if results == 0: only then insert record
            ins_news_sql = "insert into news(news_dt, city, text) values (" + dt + "," + c + "," + t + "); commit;"  # id col is autoincrement
            # cursor.execute(ins_news_sql)
        ...
        # if he wants an  add then he inputs text and date
            ...
            ins_ad_sql = "insert into ad(ad_txt, exp_date, exp_date_txt) values (" + t + "," + str(exp_date) + "," + "Days left:" + str(days_left) + "); commit;"  # id col is autoincrement
            # cursor.execute(ins_ad_sql)
            loop_marker = input("do you want to input another record: yes/no:")
            ...
            ins_fib_sql = "insert into fib(nr, fib_nr) values (" + str(n) + "," + str(nth) + "); commit;"  # id col is autoincrement
            # cursor.execute(ins_fib_sql);
            loop_marker = input("do you want to input another record: yes/no:")
        else:
    ...
    if k2 == 0 and input_file_name[-4:] == '.xml':
        ...
        while idx_lst < len(record_type):
            ...
            if record_type[idx_lst] == 'FIBONACCI':
                ...
                output_file1.write("FIBONACCI: The " + str(n) + "th fibonacci number is " + str(fn) + "\n")
            idx_lst = idx_lst + 1
    if k2 == 0 and input_file_name[-5:] == '.json':
        input_file1 = open(input_file_name, "r")
        json_data = json.load(input_file1)
        record_type_j = []
        input1_j = []
        input2_j = []
        for i in json_data['NEWSFEED']:
            for val in i:
                print("column is " + val)
                print("column value is " + i[val])
                if val == 'Record_type':
                    record_type_j.append(i[val])
                elif val == 'input1':
                    input1_j.append(i[val])
                elif val == 'input2':
                    if i[val] is not None:
                        input2_j.append(i[val])
        idx_lst2 = 0
        while idx_lst2 < len(record_type_j):
            if record_type_j[idx_lst2] == 'NEWS':
                c = input1_j[idx_lst2]
                t = fix_txt(str(input2_j[idx_lst2]))
                dt = str(datetime.now())
                n1 = News(c, t)
                output_file1.write("NEWS:      " + dt + " " + n1.show_news() + "    END RECORD" + "\n")
            if record_type_j[idx_lst2] == 'AD':
                t = fix_txt(str(input1_j[idx_lst2]))
                corr_date = 0
                try:
                    exp_date_string = input2_j[idx_lst2]
                    exp_date = datetime.strptime(exp_date_string, "%Y-%m-%d")
                    corr_date = 1
                except ValueError:
                    corr_date = 0
                ...
                if days_left == 0:
                    ...
                    days_left = " Already expired."
                output_file1.write(
                    "PRIVATE AD: " + a1.show_ad() + " .Days left: " + str(days_left) + "     END RECORD" + "\n")
            if record_type_j[idx_lst2] == 'FIBONACCI':
                n = int(input1_j[idx_lst2])
                fn = fibonacci(n - 1)
                output_file1.write("FIBONACCI: The " + str(n) + "th fibonacci number is " + str(fn) + "\n")
            idx_lst2 = idx_lst2 + 1
